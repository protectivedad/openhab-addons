...
public class HoneywellConnectionClass implements HoneywellConnectionInterface {
    ...
    public HoneywellConnectionClass(HoneywellBridgeConfig bridgeConfig, HttpClient secureClient)
        ...
        authorizationCode = bridgeConfig.authorizationCode;
        if (!authorizationCode.isEmpty()) {
            final String initRefreshToken;
            try {
                initRefreshToken = updateAccessToken();
            } catch (Exception e) {
                logger.warn("Authorization code {} failed to provide token response", authorizationCode);
                refreshToken = bridgeConfig.refreshToken;
                try {
                    updateAccessToken();
                } catch (Exception e2) {
                    logger.warn("Refresh token {} failed to provide token response", bridgeConfig.refreshToken);
    ...
    private String updateAccessToken() throws IllegalStateException {
        final Token currToken;
        try {
            final String stateContent = refreshToken.isEmpty()
                    ? "grant_type=authorization_code&redirect_uri=none&code=" + authorizationCode
                    : "grant_type=refresh_token&refresh_token=" + refreshToken;
            currToken = new Token(postTokenHoneywell(stateContent));
            accessToken = currToken.getAccessToken();
            updateAfter.setTime(new Date().getTime() + currToken.getUpdateIn());
            return currToken.getRefreshToken();
        } catch (Exception e) {
            throw new IllegalStateException("Unable to get access token.");
        }
    }
    ...
    public void refreshCache() {
        ...
        try {
            if (updateAfter.before(new Date())) {
                logger.debug("Updating access token based on time");
                updateAccessToken();
            }
